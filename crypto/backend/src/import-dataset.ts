...
type AccountRecord = { address?: unknown; chainId?: unknown; label?: unknown; nameTag?: unknown };
const VASP_LABELS: Record<string, string> = {
...
if (!datasetPath) throw new Error('Usage: npm run import-dataset -- <path-to-accounts.json>');
const records = JSON.parse(readFileSync(datasetPath, 'utf8')) as AccountRecord[];
if (!Array.isArray(records)) throw new Error('Dataset must be a JSON array.');
const unique = new Map<string, { address: string; name: string; source: string }>();
let skipped = 0;
for (const record of records) {
  const label = typeof record.label === 'string' ? record.label.toLowerCase() : '';
  if (record.chainId !== 1 || !VASP_LABELS[label]) { skipped++; continue; }
