...
function cacheDemo(sourceAddress: string, vaspAddress: string, name: string, valueWei: string, hash: string): void {
  const candidate = { name, address: vaspAddress, confidence: 100, label: 'High' as const, valueShare: 1, valueWei, hops: 1, matchType: 'exact' as const, scoreBreakdown: { hopScore: 40, valueScore: 40, matchScore: 20 } };
