...
app.post('/api/demo/multihop', async (_request, response) => {
    try {
        return response.json(await addSummary(await multiHopDemo()));
    }
    catch {
        return response.status(500).json({ error: 'Demo could not be loaded.' });
    }
});
app.post(['/api/trace', '/api/trace-wallet'], async (request, response) => {
    const address = normalizeAddress(request.body?.address);
    if (!address)
        return response.status(400).json({ error: 'Enter a valid Ethereum address.' });
    try {
        const cached = getCachedTrace(address);
        if (cached)
            return response.json(await addSummary({ ...cached, cached: true }));
...
app.post(['/api/cryptotrail/trace', '/v2/trace/deterministic'], async (request, response) => {
    ...
    if (typeof chain !== 'string' || chain.toLowerCase() !== 'ethereum')
        return response.status(400).json({ error: 'Only Ethereum tracing is currently supported.' });
    if (request.body?.max_depth_hops !== undefined && request.body.max_depth_hops !== 4)
        return response.status(400).json({ error: 'The supported depth limit is 4 hops.' });
    if (request.body?.peel_threshold !== undefined || request.body?.detectMixers === true || request.body?.resolve_exchange_tags === false)
        return response.status(400).json({ error: 'Custom peel thresholds, mixer detection and disabling exchange matching are not supported.' });
    if (request.body?.demo !== undefined && request.body.demo !== 'multihop')
        return response.status(400).json({ error: 'Unknown demo. Use multihop.' });
    try {
        if (request.body?.demo === 'multihop')
            return response.json(toCryptoTrail(await addSummary(await multiHopDemo()), performance.now() - started));
        const address = normalizeAddress(request.body?.address);
        if (!address)
            return response.status(400).json({ error: 'Enter a valid Ethereum wallet address. Transaction hashes are not supported.' });
        const cached = getCachedTrace(address);
