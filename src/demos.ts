...
export async function multiHopDemo() {
  ...
  const result = await traceWallet(demoSource, {
    ...
    findVasp: address => address === destination ? { address, name: 'Demo Exchange', matchType: 'exact' as const } : undefined
  });
  result.demo = true;
  result.nodes.forEach(node => { if (node.id === middle) node.label = 'Intermediate wallet'; });
  return result;
}
