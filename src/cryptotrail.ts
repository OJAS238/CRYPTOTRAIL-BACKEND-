...
import type { TraceResult, TraceEdge } from './types.js';

/** CryptoTrail presents nodes as a linear pipeline; select a real connected path. */
export function toCryptoTrail(result: TraceResult, elapsedMs: number) {
  const top = result.candidates[0];
  const source = result.sourceAddress.toLowerCase();
  const queue: Array<{ id: string; path: TraceEdge[] }> = [{ id: source, path: [] }];
  const visited = new Set([source]);
  let path: TraceEdge[] = [];
  let found = false;
  while (queue.length && top) {
    const current = queue.shift()!;
    if (current.id === top.address.toLowerCase()) { path = current.path; found = true; break; }
    for (const edge of result.edges.filter(edge => edge.source.toLowerCase() === current.id)) {
      const destination = edge.target.toLowerCase();
      if (!visited.has(destination)) { visited.add(destination); queue.push({ id: destination, path: [...current.path, edge] }); }
    }
  }
  ...
  if (result.truncated) warnings.push('Partial trace: the wallet or time budget was reached. Additional paths remain unexplored.');
  if (result.demo) warnings.push('Illustrative demo data; not verified on-chain.');
  if (top && !found) warnings.push('No connected path to the candidate was found in the supplied graph.');
  return {
