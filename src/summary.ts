...
export async function addSummary(result: TraceResult): Promise<TraceResult> {
  if (result.summary?.source === 'ai') return { ...result, explanation: result.summary.text };
  const top = result.candidates[0];
  const partialNotice = result.truncated ? 'Partial trace: the search limit was reached; additional paths remain unexplored. ' : ''; 
  const text = top
  ...
  const fallback = (reason: string): TraceResult => ({ ...result, explanation: 'Summary unavailable', summary: { text: partialNotice + text, source: 'template', reason } });
  const explanation = await generateExplanation(result);
  if (explanation === 'Summary unavailable') return fallback('AI summary unavailable; showing a rule-based summary.');
  return { ...result, explanation: partialNotice + explanation, summary: { text: partialNotice + explanation, source: 'ai' } };
}

/** Returns an explanation string; provider failures never reject the trace request. */
export async function generateExplanation(result: TraceResult): Promise<string> {
  const key = process.env.GROQ_API_KEY;
  const model = (process.env.GROQ_MODEL || 'openai/gpt-oss-20b');
  if (!key || !model) return 'Summary unavailable';
  try {
    const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({ model, temperature: 0.2, max_completion_tokens: 2000,
        ...
        { role: 'user', content: JSON.stringify({ partial: !!result.truncated, illustrative: !!result.demo, sourceAddress: result.sourceAddress, nodes: result.nodes, edges: result.edges, candidates: result.candidates, hiddenTransactions: result.hiddenTransactions }) }] })
    });
    ...
    if (choice?.finish_reason !== 'stop' || !summary) return 'Summary unavailable';
    return summary;
